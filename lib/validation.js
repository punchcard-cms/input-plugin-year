...
module.exports = function yearValidation(input, settings) {
  if (input.target.value && !(input.target.value >= settings.all.year.min && input.target.value <= settings.all.year.max)) {
    return `${input.target.name} should be within range!`;
  }
